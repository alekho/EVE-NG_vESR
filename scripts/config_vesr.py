#!/usr/bin/env python3
#
# scripts/config_vesr.py
#
# Import/Export script for Eltex vESR.
#
# @author Alexandr Khorev
# @copyright 2023 Alexandr Khorev
...
def config_get(handler):
    ...
    # Disable paging
    handler.sendline('terminal datadump')
    try:
        handler.expect('#', timeout = expctimeout)
    except:
        print('ERROR: error waiting for "#" prompt.')
...
def config_put(handler, config):
    ...
    # Save
    handler.sendline('confirm')
    try:
        handler.expect('confirmed', timeout = expctimeout)
    except:
        print('ERROR: error waiting for "confirmed" prompt.')
        node_quit(handler)
        return False
    handler.sendline('\r\n')
    try:
